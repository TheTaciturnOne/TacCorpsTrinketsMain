package com.thetaciturnone.taccorpstrinkets.supporter;
		...
		try {
			...
			if (connection.getResponseCode() == 200) {
				...
				if (jsonObject.has("players") && jsonObject.get("players").isJsonArray()) {
					...
					TacCorpsTrinkets.LOGGER.error("Error: 'players' field is missing or not an array!");
				}
				reader.close();
			} else {
				TacCorpsTrinkets.LOGGER.error("HTTP Error: " + connection.getResponseCode());
			}
